...
public class TaskNodeCellFactory
    implements Callback<TreeView<TaskNode>, TreeCell<TaskNode>> {
    ...
    TreeCell<TaskNode> cell = new TreeCell<TaskNode>() {
      ...
      public void updateItem(TaskNode item, boolean empty) {
        ...
        if (!empty) {
          ...
          setGraphic(iv1);
          // setGraphic(
          // new ImageView(new Rectangle(12, 12, Color.CORNFLOWERBLUE).snapshot(null, null)));
          setText(item.name);
        } else {
          setGraphic(null);
          setText(null);
  ...
  private void dragDetected(MouseEvent event, TreeCell<TaskNode> treeCell,
      TreeView<TaskNode> treeView) {
    draggedItem = treeCell.getTreeItem();

    // handle case when user drags on blank space
    if (draggedItem == null)
      return;
  ...
  private void dragOver(DragEvent event, TreeCell<TaskNode> treeCell,
      TreeView<TaskNode> treeView) {
  ...
  private void drop(DragEvent event, TreeCell<TaskNode> treeCell,
      TreeView<TaskNode> treeView) {
